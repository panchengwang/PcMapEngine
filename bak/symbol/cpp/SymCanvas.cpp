...
void SymCanvas::draw(SymText* shp) {
    ...
    // double x, y, x2, y2;
    // cairo_path_extents(_canvas, &x, &y, &x2, &y2);
    // // std::cerr << "text extent:    x: " << x << ", y: " << y << ", x2: " << x2 << ", y2: " << y2 << std::endl;
    // std::cerr << "width: " << x2 - x << ", height: " << y2 - y << std::endl;
    // double w = x2 - x;
    // double h = y2 - y;
    ...
    cairo_text_path(_canvas, shp->_text.c_str());
    cairo_restore(_canvas);
    std::cerr << "text size: " << shp->_text.size() << std::endl;
    setFillStyle(shp);
    cairo_fill_preserve(_canvas);
    setStrokeStyle(shp);
    cairo_stroke(_canvas);
...
my_png_save(void* closure,
    ...
    // 重新分配内存以容纳新数据
    unsigned char* new_data = (unsigned char*)realloc(buffer->data, buffer->size + length);
    if (!new_data) {
...
std::vector<unsigned char> SymCanvas::data() {
